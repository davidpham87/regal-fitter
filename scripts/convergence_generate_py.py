...
from regal_fit import Config, _run_sim_chunk

def main():
    print("Running Python convergence simulation...")
    cfg = Config(
        n_total=126,
        n_per_arm=63,
        enroll_bands=[
    ...
    rec = rec_strategy().example()
    print("Using generated leaky rec:", rec)
    ...
    hr_finals = [s["hr_final"] for s in accepted_stats if not np.isnan(s.get("hr_final", np.nan))]
    ...
    cur.execute("DELETE FROM convergence_py") # Clear previous runs

    cur.executemany("INSERT INTO convergence_py (hr_final) VALUES (?)", [(hr,) for hr in hr_finals])
    conn.commit()
    conn.close()

    print(f"Saved {len(hr_finals)} Python HR results to {db_path}.")
