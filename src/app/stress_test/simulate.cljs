(ns app.stress-test.simulate
  "Adaptation of regal_stress_test.py for ClojureScript, optimized for Web Worker stability."
        ...
        (let [[lo hi n] (first remaining-bands)]
          (if (> n 0)
            (do
              (dotimes [s n-sims]
                (dotimes [i n]
                  (aset enroll (+ (* s n-total) col i) (uniform-draw lo hi))))
  ...
  (let [n (.-length obs-t-arr)]
    (if (== n 0) 1.0
        (let [indices (js/Int32Array. n)]
          ...
          (.sort indices (fn [a b] (- (aget obs-t-arr a) (aget obs-t-arr b))))
      ...
      (let [offset (* s n-total)
        ...
        (dotimes [i n-total]
          (let [idx (+ offset i)
                ...
                s-val (* scale (js/Math.pow (- (js/Math.log (- 1.0 (js/Math.random)))) inv-k))
            ...
            (when (<= s-val f-upd) (aset ev-upd s (inc (aget ev-upd s))))
            (when (<= s-val f-pr3) (aset ev-pr3 s (inc (aget ev-pr3 s))))

            (aset trial-obs-t i (js/Math.min s-val f-ia))
            (aset trial-is-ev i (if (<= s-val f-ia) 1 0))))

        ;; KM for pooled median at IA
        (let [s-at-12 (km-survival-single trial-obs-t trial-is-ev pool-mos-min)]
          (when (> s-at-12 0.5)
            (aset pass-pool s 1)))))
    ...
    (let [obs-ev-ia (:obs-ev-ia config)
      ...
      (dotimes [s n-sims]
        (let [e-ia (aget ev-ia s)
              ...
              pass-hr (< g-ia (* futility-hr-max (/ e-ia (inc futility-hr-max))))
          ...
          (let [c1 (<= e-ia obs-ev-ia)
            ...
            (when (and passed-ia c1 c2 c3) (swap! joint-pass-count inc)))))

      (let [exp-ev-ia (/ @sum-ev-ia n-sims)
            exp-inc-upd (/ @sum-inc-upd n-sims)
            exp-inc-pr3 (/ @sum-inc-pr3 n-sims)
            residual (js/Math.max (js/Math.abs (- exp-ev-ia obs-ev-ia))
