(ns app.ui.enrollment
  (:require [app.components.editor :refer [code-editor]]
...
(defn- s-curve-mode-view [enrollment-mode]
        ...
        [:div.grid.grid-cols-2.gap-4
         [:div
          [:label.block.text-sm.font-semibold.text-gray-700
           "Median Month"]
          [inputs/num-input props :median-month inputs/parse-double-safe]]
         [:div
          [:label.block.text-sm.font-semibold.text-gray-700
           "Logistic k"]
          [inputs/num-input props :k inputs/parse-double-safe]]]
        [:button.mt-4.bg-blue-600.text-white.px-4.py-2.rounded.font-semibold
         {:type "submit" :class "hover:bg-blue-700"}
         "Generate Bands"]])]))

(defn- toolbar-view [enrollment-mode bands]
   ...
   [:div.flex.items-center.gap-2.bg-gray-100.p-2.rounded-lg.border
    ...
    [:input.border.p-1.rounded.w-12.text-center.text-xs
     {:type "number" :min "1" :max "20"
      :value (:window-param enrollment-mode)
      :on-change #(rf/dispatch [:set-enrollment-mode-param
                                :window-param
                                (js/parseInt (.. % -target -value) 10)])}]
    [:button.px-3.py-1.rounded.text-sm.font-semibold
     {:class "bg-amber-600 text-white hover:bg-amber-700"
      :on-click (fn []
                  (let [w (or (:window-param enrollment-mode) 2)
                        new-bands (rfe/larger-bands bands w)]
                    (rf/dispatch [:set-config-key
...
(defn enrollment-view []
  (fn []
    (let [config @(rf/subscribe [:config])
          ...
          seed (:seed config)
          n-samples (:n-samples enrollment-mode)
      ...
      [:div.p-4.max-w-6xl.mx-auto
       ...
       [toolbar-view enrollment-mode bands]
       (if (= (:mode enrollment-mode) :manual)
         [manual-mode-view bands]
         [s-curve-mode-view enrollment-mode])
