shadow$provide.module$node_modules$d3_timer$src$timer = function(require, module, exports) {
  function now() {
  ...
  function sleep(time) {
    frame || (timeout &&= clearTimeout(timeout), time - clockNow > 24 ? (time < Infinity && (timeout = setTimeout(wake, time - clock.now() - clockSkew)), interval &&= clearInterval(interval)) : (interval || (clockLast = clock.now(), interval = setInterval(poke, 1000)), frame = 1, setFrame(wake)));
  }
  ...
  var frame = 0, timeout = 0, interval = 0, taskHead, taskTail, clockLast = 0, clockNow = 0, clockSkew = 0, clock = typeof performance === "object" && performance.now ? performance : Date, setFrame = typeof window === "object" && window.requestAnimationFrame ? window.requestAnimationFrame.bind(window) : function(f) {
    setTimeout(f, 17);
  };
